...
def new_ss_id_single_plot(df, dataset='pre'):
    # load from df
    if dataset=='pre':
    ...
    id_means = df.groupby('model_name').ID_pre.apply(lambda g: np.mean(g.values.tolist(), axis=0))
    id_stds = df.groupby('model_name').ID_pre.apply(lambda g: np.std(g.values.tolist(), axis=0))

    ss_means = df.groupby('model_name').SS_pre.apply(lambda g: np.mean(g.values.tolist(), axis=0))
    ss_stds = df.groupby('model_name').SS_pre.apply(lambda g: np.std(g.values.tolist(), axis=0))

    for i in id_means.index:
...
if __name__ == '__main__':
    ...
    # dataset_extracted = 'mnist'  # 'fashionmnist'
    ...
    if model_folder == 'all':
        ...
        #     paths.append(join(models_dir, 'models_' + str(seed), 'ss_id.json')) #list of paths
        ...
        path = join(models_dir, 'models_' + model_folder, 'ss_id.json')
