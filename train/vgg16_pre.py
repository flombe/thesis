...
# fix seed (run for random model for multiple seeds (see bottom))
seed = 1
...
def get_vgg16bn_segnet():
    # weights from prev. student at TU Berlin
    ...
    for key, key2 in zip(checkpoint['model_state_dict'].keys(), model.state_dict().keys()):
        if str(key).startswith('encoder.'):
            new_state_dict[key2] = checkpoint['model_state_dict'][key]
        else:
...
if __name__=='__main__':
    ...
    # set to true to load, save and test model
    ...
    if init_segnet:
        ...
        # weights from other Thesis at NI chair
        ...
        # model test not useful, since classifier not trained
